...
Backend now persists a runtime registry for long-lived owners, while Executor Manager runs an idle controller that can let a runtime go `running -> warm_idle -> sleeping` without deleting its durable state.
...
For operators, the key product change is that "persistent" no longer means "the container must stay alive forever". Instead, Poco preserves the runtime owner, workspace, `/agent_state`, and session anchor, and only keeps the compute hot when there is active work or an explicit pin lease.
