...
class McpPresetService:
    ...
    ) -> list[McpPresetResponse]:
        presets = McpPresetRepository.list_visible(
    ...
    def get_preset(
        ...
        preset = McpPresetRepository.get_by_id(db, preset_id)
        if not preset or (
            preset.source != "system" and preset.owner_user_id != user_id
        ):
            raise AppException(
                error_code=ErrorCode.MCP_PRESET_NOT_FOUND,
                message=f"MCP preset not found: {preset_id}",
            )
        return self._to_response(preset)

    def create_preset(
        self, db: Session, user_id: str, request: McpPresetCreateRequest
    ) -> McpPresetResponse:
        if McpPresetRepository.get_by_name(db, request.name):
            raise AppException(
                error_code=ErrorCode.BAD_REQUEST,
                message=f"MCP preset already exists: {request.name}",
            )
        ...
        McpPresetRepository.create(db, preset)
        db.commit()
        db.refresh(preset)
        return self._to_response(preset)
    ...
    def update_preset(
        ...
        if preset.source == "system":
            raise AppException(
                error_code=ErrorCode.MCP_PRESET_CREATE_FORBIDDEN,
                message="Cannot modify system MCP presets",
            )
        if preset.owner_user_id and preset.owner_user_id != user_id:
            raise AppException(
                error_code=ErrorCode.FORBIDDEN,
                message="MCP preset does not belong to the user",
            )
        ...
        if request.transport is not None:
            preset.transport = request.transport
    ...
    def delete_preset(self, db: Session, user_id: str, preset_id: int) -> None:
        ...
        if preset.source == "system":
            raise AppException(
                error_code=ErrorCode.MCP_PRESET_CREATE_FORBIDDEN,
                message="Cannot delete system MCP presets",
            )
        if preset.owner_user_id and preset.owner_user_id != user_id:
            raise AppException(
                error_code=ErrorCode.FORBIDDEN,
        ...
        McpPresetRepository.delete(db, preset)
        db.commit()

    @staticmethod
    def _to_response(preset: McpPreset) -> McpPresetResponse:
        return McpPresetResponse(
            id=preset.id,
            name=preset.name,
