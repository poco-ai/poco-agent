...
- [x] Phase 1: 用 library-style shell 重构 overview / members / invites / issues
- [x] Phase 2: 清理视觉噪音并修正纵向间距
- [x] Phase 3: 补齐移动端行为、状态与验收
...
- [x] 顶部圆角 pills 导航被移除
- [x] 左侧 rail 具备明确的 active / hover / mobile 行为
- [x] workspace 上下文信息不再喧宾夺主
...
- [x] overview、members、invites、issues 的右侧内容区结构一致
- [x] toolbar 位置稳定，不再有页面把 action 塞到顶部 summary card 里
- [x] issues section 也属于 team detail pane，而不是视觉上脱离 team 模块
...
- [x] team overview 不再使用大面积渐变背景
- [x] 所有 team section 的主要表面统一为 token 驱动的 card / muted / border 样式
- [x] 页面主层级依靠 spacing 和 typography，而不是背景特效
...
- [x] “邀请 / 查看待处理和已撤销的空间邀请。” 上方不再显得贴边
- [x] 所有 section 标题块与容器顶边之间的留白稳定一致
- [x] detail pane 内 section 间距由共享 shell 控制，而不是散落在各个页面组件里
...
- [x] 移动端 team 首先展示 section list
- [x] 点击某个 section 后进入 detail view
- [x] 返回行为清晰，不会让用户困在 detail 内
...
- [x] `pnpm lint` 通过
- [x] `pnpm build` 通过
- [x] team 关键 section 的桌面端与移动端完成手动检查
