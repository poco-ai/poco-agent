...
- [x] Phase 1: 重构 issues 为内部状态驱动的 index→detail 模式
- [x] Phase 2: 清理废弃路由和冗余代码
- [x] Phase 3: 移动端对齐与验收
...
- [x] `selectedIssueId` 为 `null` 时右侧显示 issue index
- [x] `selectedIssueId` 有值时右侧显示 issue detail
- [x] URL query param `?issue=<id>` 能持久化选中状态
- [x] 左侧 rail 始终选中 "Issues"
...
- [x] detail 视图顶部有明确的返回按钮
- [x] 点击返回后回到 issue index，board 和 issue 列表保持之前的选中状态
- [x] 移动端同样显示返回按钮
...
- [x] `/team/issues/[issueId]` 路由不再存在
- [x] 从外部链接到 issue 详情使用 `/team/issues?issue=<id>` 格式
- [x] `TeamIssueDetailPageClient` 仍可复用，只是渲染方式从路由级改为 pane 内切换
...
- [x] barrel export 不再导出 `TeamInvitesPageClient`
- [x] 无未使用的导入或导出
...
- [x] 无仅被已删除页面使用的孤立 i18n key
- [x] members dialog 中使用的 invites key 保持不变
...
- [x] 移动端 section list 只显示 3 个入口
- [x] 点击 Members / Issues 进入 detail，有返回按钮
- [x] 返回按钮回到 section list
...
- [x] 移动端 issues index 正常显示 board 列表和 issue 列表
- [x] 点击 issue 进入 detail，有返回按钮
- [x] 返回按钮回到 issue index
...
- [x] `pnpm lint` 通过
- [x] `pnpm build` 通过
- [x] 新增或调整的用户文案全部完成 i18n 覆盖
- [x] 更新 `05-workspace-team-library-layout-plan.md` 中相关验收状态
