...
- [x] Phase 2: 重做 team issues index 与 board 导航
- [x] Phase 3: 重做 issue detail 与 execution 区域
- [x] Phase 4: 补齐状态、移动端与验收
...
- [x] team overview、members、invites、issues 共享同一套 header 和 section nav
- [x] issues 路由不再脱离 team shell 单独渲染一套页头
- [x] team 相关路由在桌面端和移动端都有统一容器边距与 section gap
...
- [x] 页头右侧不再常驻 workspace 创建输入框
- [x] issues index 不再同时展示 board 创建输入和 issue 创建输入的两排 inline form
- [x] 新建动作都有统一入口按钮和独立承载容器
...
- [x] overview 首屏能在不滚动的情况下让用户理解当前 workspace 上下文
- [x] summary cards、activity list、empty / loading state 层级清晰
- [x] refresh 和主动作都位于一致的 toolbar / section header 中
...
- [x] members 与 invites 的列表项密度和状态表达一致
- [x] role / revoked / expired 等状态不再以裸文本直接输出
- [x] 新建 invite 使用 dialog 或 sheet，而不是占据内容区首行
...
- [x] board 选择与 issue 浏览在视觉上明确分层
- [x] 当前 board 的名称、说明、主动作不再淹没在列表里
- [x] issue list 支持至少一层轻量筛选或排序入口
...
- [x] issue list item 的信息密度提升，但仍保持可扫描
- [x] create issue 不再通过列表顶部 inline input 完成
- [x] 空 board、空 issue、加载中三种状态有清晰差异
...
- [x] issue narrative 与 execution controls 不再混在同一个长表单流中
- [x] session、container、assignment status 具备稳定的信息区块
- [x] detail 页首屏能直接回答“这是什么问题”“当前 AI 在做什么”
...
- [x] detail 页存在容纳 impact preview 和 next run 的明确区域
- [x] 高风险动作有确认容器，不依赖 toast 解释副作用
- [x] scheduled 与 persistent 两种 trigger mode 的差异能在 UI 上被看见
...
- [x] loading / empty / error / refreshing / destructive action 有统一表达
- [x] 用户可见状态文本经过格式化，不直接输出原始枚举
- [x] 所有新增文案完成 i18n 覆盖
...
这一轮 UI 改造完成后，需要把自动化验证结果和剩余风险明确记录下来，避免 spec 进入“看起来完成了，但没有证据”的状态。

- 2026-04-16：`node --test --experimental-strip-types --experimental-specifier-resolution=node features/workspaces/lib/team-sections.test.ts`
- 2026-04-16：`node --test --experimental-strip-types --experimental-specifier-resolution=node features/workspaces/lib/team-overview.test.ts`
- 2026-04-16：`node --test --experimental-strip-types --experimental-specifier-resolution=node features/issues/lib/issues-index-view.test.ts`
- 2026-04-16：`node --test --experimental-strip-types --experimental-specifier-resolution=node features/issues/lib/issue-detail-view.test.ts`
- 2026-04-16：`node --test --experimental-strip-types --experimental-specifier-resolution=node features/issues/lib/issue-presentation.test.ts`
- 2026-04-16：`pnpm exec eslint features/workspaces/lib/team-sections.ts features/workspaces/lib/team-overview.ts features/workspaces/ui/team-shell.tsx features/workspaces/ui/team-pages.tsx features/issues/lib/issues-index-view.ts features/issues/lib/issue-detail-view.ts features/issues/lib/issue-presentation.ts features/issues/lib/issue-presentation.test.ts features/issues/ui/issues-pages.tsx`
- 2026-04-16：`pnpm lint`
- 2026-04-16：`pnpm build`
- 说明：本次会话未启动完整 backend / executor / executor_manager 联调环境，因此未记录真实数据下的人工路径回归结果；这部分仍需要在本地多服务环境中补跑一次。
