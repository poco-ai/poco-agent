...
- Task activity 由 task 绑定的 channel thread 形成 timeline；一条 thread 对应一个 task，task 的轻量 event 和 agent 回复都追加为该 thread 的 replies。
...
当前实现已经开始把 task 变化写成轻量 `event` message，而不是普通人类消息。创建 task、状态变更、claim/unclaim 等事件已经进入 task thread。但事件 payload 仍偏技术字段：只有 actor 和粗略 assignee payload，缺少稳定的“委托方 / 被委托方”展示模型；前端 task 面板也还没有稳定地把 task 对应的 root thread 作为唯一追踪入口。
...
Alice 在 `#Public` 里发送一条消息并标记为 task，标题是“整理 onboarding 文档”，暂时不分配给任何人。该消息成为 task `#42` 的 root thread，频道主 timeline 仍显示这条 root 消息。Task 创建、状态变化、评论和后续 agent 回复都作为 replies 追加到同一个 thread。

Bob 打开 task 面板，把 assignee 改成 agent `@docs-agent`。Thread 中追加一条轻量事件：“Bob assigned task #42 to @docs-agent: 整理 onboarding 文档”。Task 面板更新为 Bob 委托、`@docs-agent` 被委托，头像和名称都可见。

`@docs-agent` 在执行过程中通过结构化 task tool 把 task 移到 `in_progress`，随后评论“已完成初稿”。这些变化都追加为 task thread 的 replies。Alice 从 task 面板打开该 task 时，右侧抽屉展示的就是 root thread，而不是另一个上下文抽屉。

Carol 也可以创建一个完全不绑定 agent 的 task `#43`。它仍然是有效 task，后续可以由人认领，也可以稍后再委托给某个 agent。

## 最终决策

Channel task 的核心心智是“频道内可追溯的协作工作项”。它可以由人或 agent 创建，可以被委托给人或 agent，也可以保持未分配。所有重要变化都必须通过 channel event 进入 task 绑定 thread，task activity 则是这个 thread 的投影。
...
- **技术决策**：task 主表继续以 UUID 作为主键，但新增或暴露 channel-local display number，用于 UI 中的 `#xxx`。`thread_root_message_id` 指向该 task 绑定的 root message；Task event message 是该 thread 的 replies，不另建并行 activity log。
...
- Task activity 必须可追溯到 channel thread message；不能只存在 task 表的 `updated_at` 或前端本地状态中。
- Task event 在 task thread 中使用轻量提示样式，不渲染成普通聊天气泡。
- 一条 thread 最多对应一个 task。用户用 As task 创建的消息就是该 task 的 root；对已有 thread 中的 reply 创建 task 时，task 绑定到该已有 root thread。
...
- `thread_root_message_id`：task 绑定的 root message。As task 创建时指向原用户消息；无来源创建时指向后端创建的 `task.created` root event。
...
erDiagram
    ...
    SERVER_CHANNEL_TASK ||--o{ SERVER_CHANNEL_MESSAGE : "activity replies through thread"
...
Task event 是一行轻量提示，内容结构需要支持 task thread activity。
...
文案最多一行；task title 或 comment 过长时截断。完整内容在 task 面板展示的 root thread 中查看。
...
4. 如果 task 来自 As task 消息或已有 thread，`server_channel_tasks.thread_root_message_id` 指向该 root message，并把 `task.created` 作为 reply 写入该 thread。
5. 如果 task 没有来源消息，后端创建 `task.created` root event，并让 `server_channel_tasks.thread_root_message_id` 指向该 event。
6. 后续 task activity event 和 agent execution projection 都以 `thread_root_message_id` 挂到同一个 thread。
7. 前端 task board/list 读取 task 主表，展示当前状态。
8. 前端 task 面板用 `thread_root_message_id` 读取并展示完整 thread。
...
Task 面板的 activity 不应直接显示 raw message type badge。它应直接读取并展示 `thread_root_message_id` 对应的 root thread，复用 channel thread 的 message/event 渲染逻辑。用户点击 task 面板里的 task 后，右侧抽屉显示这个 root thread，而不是另开 task event 的上下文抽屉。
...
Thread API 继续返回 message 列表，但 event content 必须稳定可解析。
...
- **方案 B：task activity 使用独立 activity log 表，不进入 channel messages。**
  不采用作为第一选择。独立 activity log 对审计有价值，但会切断 task 和 channel thread 的自然关系。当前更重要的是让活动固定在 task 的 root thread 中。
...
flowchart TD
    ...
    D --> E["Task thread lightweight reply"]
    D --> F["Task panel root thread"]
    F --> G["Show root message and replies"]
```
...
- 当前前端已有 server conversation drawer 体系；task 面板应复用 thread drawer 的布局心智，不单独跳转整页。
...
| 2026-05-11 | 修正为一 thread 一 task | As task 原消息作为 task root，task 更新和 agent 回复统一追加到 root thread；task 面板点击直接展示 root thread |
