...
- [ ] issues 页不再渲染左侧 board 列表卡作为主结构
- [ ] 当前 board 名称、描述和 actions 都集中在顶部 context bar
- [ ] summary metrics 仍保留，但位置从右侧内容区内迁移到 board context
...
- [ ] 当前 board 的 issue 以多列方式展示，而不是单列列表
- [ ] 每列 header 显示状态名称和 issue 数量
- [ ] 空列有清晰的 empty state，不退化为留白
- [ ] 现有 AI assignment、priority、project 等信息能在卡片上被快速扫描
...
- [ ] 移动端不会出现不可读的横向缩略列
- [ ] 用户可以明确切换当前查看的状态列
- [ ] 创建 issue、打开卡片、返回 board 的路径在移动端保持通顺
