...
- [x] Phase 2: 将 issue 详情改为 modal / sheet
- [x] Phase 3: 接入拖拽排序与状态流转
...
- [x] 点击 issue card 会打开 dialog，而不是切走主画布
- [x] 关闭 dialog 后仍保留当前 board、列和滚动位置
- [x] `?issue=<id>` 仍可用于深链打开 detail，但表现为 modal 而非整页
...
- [x] overview、assignment、execution 的层级比当前更清晰
- [x] issue 删除入口存在且带确认提示
- [x] open session、trigger、retry、cancel、release 仍可在 detail 中操作
...
- [x] 卡片可在同一列内重排
- [x] 卡片可拖到其他状态列
- [x] 拖拽过程中列容器和卡片有清晰的 drop affordance
- [x] 不引入新的拖拽依赖
...
- [x] 当前 board 有明确的 settings 入口
- [x] 可在 dialog 中修改 board 名称和描述
- [x] 保存成功后 context bar 和画布上下文同步刷新
...
- [x] 删除确认文案明确说明 issue 会被一并删除
- [x] 删除前能展示当前 board 的 issue 数量
- [x] 删除成功后自动切换到下一个可用 board；若无 board，则回到空态
...
- [x] backend 相关测试覆盖跨列移动、列内重排和删除
- [x] frontend 纯函数测试覆盖分列、排序和 optimistic rollback 所需数据转换
- [x] `pnpm lint` 和 `pnpm build` 可通过
- [ ] 手工验证以下路径：
- [ ] 切换 board
- [x] 创建 issue
- [x] 点击卡片打开 modal
