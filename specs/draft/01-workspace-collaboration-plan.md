...
**验收标准：**
- [ ] spec 中明确 shared resource 不再只靠 `user_id` 表达归属
- [ ] spec 中明确创建者和时间信息为必备展示属性
...
**验收标准：**
- [ ] spec 中明确 preset 的三类 scope
- [ ] spec 中明确 shared preset 需要展示创建者与创建时间
- [ ] spec 中明确 preset 仍沿用现有 preset 领域 API，不另起平行资源
...
**验收标准：**
- [ ] spec 中明确是复制 / 派生而不是实时同步
- [ ] spec 中明确复制后保留 `forked_from` 来源追踪
- [ ] spec 中明确 request/response 使用显式 schema 命名
...
**验收标准：**
- [ ] spec 中明确 shared project 第一版不直接绑定用户本地挂载
- [ ] spec 中明确团队 project 允许直接执行 agent 任务
- [ ] spec 中明确团队 project 的基础元数据
...
**验收标准：**
- [ ] spec 中明确复制行为的目标空间和 ownership 变化
- [ ] spec 中明确不做实时双向同步
- [ ] spec 中明确 project file 与 project metadata 的复制范围
- [ ] spec 中明确复制后保留 `forked_from` 来源追踪
...
**验收标准：**
- [ ] spec 中明确 session 仍然 user-owned
- [ ] spec 中明确共享 project 只影响资源选择和授权校验
- [ ] spec 中明确第一版不要求 executor 协议感知 workspace
...
**验收标准：**
- [ ] spec 中明确 board 属于 workspace
- [ ] spec 中明确 board 是 issue 的容器，而不是 project 的别名
...
**验收标准：**
- [ ] spec 中明确 issue 的系统字段集合
- [ ] spec 中明确双模 assignee 互斥约束
- [ ] spec 中明确 assignee 切换时旧 assignee 自动清除
...
**验收标准：**
- [ ] spec 中明确 issues feature 目录结构
- [ ] spec 中明确 route 文件保持薄
- [ ] spec 中明确新增文案必须接入 i18n
...
**验收标准：**
- [ ] spec 中明确每类资源的默认访问策略
- [ ] spec 中明确 owner/admin/member 对资源策略的影响
...
**验收标准：**
- [ ] spec 中明确 board 操作审计点
- [ ] spec 中明确 metadata 包含 board 名称等关键信息
...
**验收标准：**
- [ ] spec 中明确 issue 操作审计点
- [ ] spec 中明确 assignee 类型在 metadata 中的区分方式
...
**验收标准：**
- [ ] spec 中明确 project 操作审计点
- [ ] spec 中明确复制/派生操作记录来源和目标信息
